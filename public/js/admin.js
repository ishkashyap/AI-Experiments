...
if (loginForm) {
    loginForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            messageDiv.innerHTML = `<div class="text-red-500 font-medium p-3 bg-red-50 rounded">${err.message}</div>`;
        } finally {
    ...
    try {
        ...
        tbody.innerHTML = response.data.map(blog => `
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-6 py-4 font-medium text-slate-900">${blog.title}</td>
    ...
    try {
        ...
        tbody.innerHTML = data.map((booking, idx) => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-4 text-sm text-slate-700">${booking.service}</td>
                <td class="px-4 py-4">
                    <div class="text-sm font-medium text-slate-800">${booking.date}</div>
...
async function saveCredentials() {
    ...
    try {
        ...
        msgDiv.innerHTML = `<div class="text-red-500 font-medium p-3 bg-red-50 rounded">${err.message}</div>`;
    }
}
...
async function loadAnnouncements() {
    try {
        ...
        tbody.innerHTML = response.data.map(ann => `
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-6 py-4 font-medium text-slate-900">${ann.title}</td>
                <td class="px-6 py-4"><span class="bg-gray-100 px-2 py-1 rounded text-xs">${ann.type}</span></td>
                <td class="px-6 py-4">${ann.content.substring(0, 50)}...</td>
    ...
    try {
        ...
        tbody.innerHTML = data.map((contact, idx) => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-6 py-4">
                    <span class="status-badge status-pending">${contact.subject || 'General'}</span>
                </td>
                <td class="px-6 py-4 max-w-xs truncate text-sm text-slate-600">${contact.message || '-'}</td>
...
function openContactDetail(idx) {
    ...
    document.getElementById('contact-detail-email').innerHTML = contact.email
        ? `<a href="mailto:${contact.email}" class="text-navy hover:underline">${contact.email}</a>`
        : 'N/A';
    ...
    try {
        ...
        tbody.innerHTML = response.data.map(member => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-6 py-4 text-gray-500">${member.description ? member.description.substring(0, 50) + '...' : '-'}</td>
                <td class="px-6 py-4 text-center">
...
function createServiceCardHTML(num, data) {
    ...
    return `
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <input type="text" id="svc-${num}-image" value="${(d.image || d.content || '').replace(/"/g, '&quot;')}" placeholder="https://example.com/image.jpg" class="input-premium flex-1 px-3 py-2 rounded-lg text-sm">
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <input type="text" id="svc-${num}-tagline" value="${(d.tagline || '').replace(/"/g, '&quot;')}" placeholder="Expert Representation" class="input-premium flex-1 px-3 py-2 rounded-lg text-sm">
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <input type="text" id="svc-${num}-icon" value="${(d.icon || '').replace(/"/g, '&quot;')}" placeholder="⚖️" class="input-premium flex-1 px-3 py-2 rounded-lg text-sm">
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <input type="text" id="svc-${num}-title" value="${(d.title || '').replace(/"/g, '&quot;')}" placeholder="Service Title" class="input-premium flex-1 px-3 py-2 rounded-lg text-sm">
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <textarea id="svc-${num}-desc" rows="2" placeholder="Service description..." class="input-premium flex-1 px-3 py-2 rounded-lg text-sm resize-none">${(d.desc || '').replace(/"/g, '&quot;')}</textarea>
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-2">
                <textarea id="svc-${num}-points" rows="3" placeholder="Point 1&#10;Point 2&#10;Point 3&#10;Point 4" class="input-premium flex-1 px-3 py-2 rounded-lg text-sm resize-none">${(d.points || []).join('\n')}</textarea>
