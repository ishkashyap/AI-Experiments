...
function displayLatestBlogs(blogs) {
    ...
    container.innerHTML = blogs.map(blog => `
        <div class="card-luxury rounded-2xl overflow-hidden hover-lift cursor-pointer" onclick="viewBlog(${blog.id})">
            ${blog.image ? `<img src="${blog.image}" alt="${blog.title}" class="w-full h-48 object-cover" loading="lazy">` : ''}
            <div class="p-6">
                <h3 class="font-serif text-xl font-bold text-navy mb-3">${blog.title}</h3>
                <p class="text-slate-500 font-light text-sm mb-4">${blog.content.substring(0, 100)}...</p>
...
function displayAllBlogs(blogs) {
    ...
    container.innerHTML = blogs.map(blog => `
        <div class="border border-slate-200 rounded-2xl overflow-hidden hover:-translate-y-1 transition-transform cursor-pointer shadow-sm" onclick="viewBlog(${blog.id})">
            ${blog.image ? `<img src="${blog.image}" alt="${blog.title}" class="w-full h-56 object-cover" loading="lazy">` : ''}
            <div class="p-8">
                <div class="flex items-center space-x-4 mb-4 text-sm text-slate-500">
                    <span>📅 ${new Date(blog.created_at).toLocaleDateString()}</span>
                    <span>✍️ ${blog.author}</span>
                </div>
                <h3 class="font-serif text-2xl font-bold text-navy mb-4">${blog.title}</h3>
                <p class="text-slate-500 font-light mb-6">${blog.content.substring(0, 150)}...</p>
...
async function viewBlog(id) {
    try {
        ...
        detailDiv.querySelector('#blogContent').innerHTML = `
            ${blog.image ? `<img src="${blog.image}" alt="${blog.title}" class="w-full h-96 object-cover rounded-2xl mb-8">` : ''}
            <div class="animate-fadeInUp">
                <h1 class="font-serif text-4xl md:text-5xl font-bold text-navy mb-6">${blog.title}</h1>
    ...
    try {
        ...
        annContainer.innerHTML = announcements.map(ann => `
            <div class="inline-block px-4 py-2 rounded-full text-sm font-medium ${getAnnouncementBadge(ann.type)}">
                ${ann.type.toUpperCase()}: ${ann.title} - ${ann.content || ''}
            </div>
        `).join('');
    } catch (err) {
        ...
        validItems.forEach(item => {
            ...
            if (item.page === 'booking') {
                if (item.section.startsWith('booking-note-') && document.getElementById(`content-${item.page}-${item.section}`)) {
                    document.getElementById(`content-${item.page}-${item.section}`).innerHTML = item.content;
                }
                ...
                if (bookingNoteEl && item.section.startsWith('note-')) {
                    bookingNoteEl.innerHTML = item.content;
                }
...
if (item.section === 'hours-sunday' && document.getElementById('contact-hours-sunday')) {
        ...
        nums.forEach(num => {
            ...
            section.innerHTML = `
                <div class="lg:w-1/2">
                    <div class="relative">
                        <img id="home-service-${num}-image" src="${imgSrc}" alt="${titleContent}" class="rounded-2xl shadow-2xl w-full aspect-[4/3] object-cover">
                    </div>
                </div>
                <div class="lg:w-1/2">
                    <span id="services-${num}-tagline" class="text-gold font-semibold text-sm tracking-[0.3em] uppercase">${taglineContent}</span>
                    <h2 id="services-${num}-title" class="font-serif text-4xl font-bold text-navy mt-4 mb-6">${titleContent}</h2>
                    <div class="section-divider ml-0 mb-6"></div>
                    <p id="services-${num}-desc" class="text-gray-600 text-lg leading-relaxed mb-8">${descContent}</p>
        ...
        function fillServicePoints(num, data, items) {
            ...
            for (let pn = 1; pn <= 4; pn++) {
                ...
                if (content) {
                    ...
                    div.innerHTML = `<span class="text-gold text-xl">◆</span><span class="text-gray-700">${content}</span>`;
            ...
            container.innerHTML = members.map(member => {
                ...
                <div class="card-luxury rounded-2xl overflow-hidden hover-lift">
                    <div class="relative overflow-hidden">
                        <img src="${imgSrc}" alt="${member.name}" class="w-full h-64 object-cover">
                    ...
                    <div class="p-6 text-center">
                        <h3 class="font-serif text-xl font-bold text-navy mb-1">${member.name}</h3>
                        <p class="text-gold font-semibold text-sm mb-3">${member.position}</p>
